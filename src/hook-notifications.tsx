...
const NotificationsDispatchContext = createContext<React.Dispatch<NotificationActions> | undefined>(undefined)

const notificationReducer = (state: Notification[], action: NotificationActions) => {
...
export const Notifications: React.FC = ({children}) => {
  ...
  return <NotificationsStateContext.Provider value={state}>
...
export const useNotifications: useNotificationsFunction = () => {
  ...
  if(!dispatch){
    throw new Error("useNotifications can only be called within <Notifications>")
  }
